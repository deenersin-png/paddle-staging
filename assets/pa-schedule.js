...
// ---- a week the operator typed --------------------------------------------
// Vacation is picked by the week, and an operator names a week by a date in
// it: "27/9" for next week, "10/18" for one further out. Both orders turn up,
// so both are read: a part over 12 can only be the day, and when both
// readings are real dates the one that lands on a Sunday wins — an operator
// names the week by the Sunday it starts. Ties go to month/day (US).
// The year is left out too, so it is the one that puts the date inside
// [today - 90 days, today + 274 days]: exactly one year can, and a week just
// gone is as legitimate an answer as one next spring.
...
/** The year that puts month/day nearest today's window, or null if unreal. */
function yearFor(m, d, today) {
  const t0 = fromIso(today).getTime(), base = fromIso(today).getFullYear();
  let best = null;
  for (const y of [base - 1, base, base + 1]) {
    if (!isRealDate(y, m, d)) continue;
    const diff = (fromIso(isoOf(y, m, d)).getTime() - t0) / 86400000;
    if (diff >= -90 && diff <= 274) return y;                  // in the window: done
...
/**
 * Read a week the operator typed. Accepts "27/9", "9/27", "10/18", "Sep 27",
 * "27 sept", "2026-09-27", with or without a year, and "this week" / "next
 * week".
 *
 * Returns { weekStart, date, ambiguous } — weekStart is the Sunday the week
 * begins, `date` the date actually read, `ambiguous` true when the other
 * order was also a real date — or null when nothing sensible was typed.
 */
export function parseWeekInput(text, today) {
  const t = String(text == null ? '' : text).trim().toLowerCase();
  const now = today || todayIso();
  if (!t) return null;

  const week = (iso, ambiguous) => ({ weekStart: weekStartOf(iso), date: iso, ambiguous: !!ambiguous });

  if (/^(this|current)\s*(week)?$/.test(t)) return week(now, false);
  if (/^next\s*(week)?$/.test(t)) return week(addDays(weekStartOf(now), 7), false);

  // 2026-09-27 (explicit, nothing to guess)
  let m = t.match(/^(\d{4})-(\d{1,2})-(\d{1,2})$/);
  if (m) {
    const [y, mo, d] = [+m[1], +m[2], +m[3]];
    return isRealDate(y, mo, d) ? week(isoOf(y, mo, d), false) : null;
  }

  // "sep 27", "27 september", with an optional year
  m = t.match(/^([a-z]{3,9})\.?\s+(\d{1,2})(?:[,\s]+(\d{2,4}))?$/)
   || t.match(/^(\d{1,2})\s+([a-z]{3,9})\.?(?:[,\s]+(\d{2,4}))?$/);
  if (m) {
    const name = /^[a-z]/.test(m[1]) ? m[1] : m[2];
    const day  = +(/^[a-z]/.test(m[1]) ? m[2] : m[1]);
    const mo = MONTHS.findIndex(n => n.startsWith(name.slice(0, 3))) + 1;
    if (!mo) return null;
    const y = m[3] ? fullYear(+m[3]) : yearFor(mo, day, now);
    return y && isRealDate(y, mo, day) ? week(isoOf(y, mo, day), false) : null;
  }

  // 27/9 · 9/27 · 10-18 · 9/27/2026
  m = t.match(/^(\d{1,2})\s*[\/\-. ]\s*(\d{1,2})(?:\s*[\/\-. ]\s*(\d{2,4}))?$/);
  if (!m) return null;
  const a = +m[1], b = +m[2], givenYear = m[3] ? fullYear(+m[3]) : null;
  // Both readings, month/day first.
  const reads = [{ mo: a, d: b }, { mo: b, d: a }]
    .map(r => {
      const y = givenYear != null ? givenYear : yearFor(r.mo, r.d, now);
      return y && isRealDate(y, r.mo, r.d) ? { ...r, iso: isoOf(y, r.mo, r.d) } : null;
    })
    .filter(Boolean);
  if (!reads.length) return null;
  if (reads.length === 1) return week(reads[0].iso, false);
  if (a === b) return week(reads[0].iso, false);                // 9/9 reads the same either way
  const sunday = reads.find(r => dowOf(r.iso) === 0);
  return week((sunday || reads[0]).iso, true);
}

/** "26" -> 2026, "126" -> 2126, four digits as given. */
function fullYear(n) { return n >= 100 ? n : 2000 + n; }
